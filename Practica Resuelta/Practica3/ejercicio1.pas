...
type 
    ...
    procedure leerArchivo(var a: archivo_empleados, var dato: empleadoRegistro);
    begin
        if (not EOF(a)) then    
            read(a, dato)
        else 
    ...
    procedure eliminarEmpleado (var arch: empleados);
    var
        numEmpleado, posEncontrado, posUltimo: integer;
        regEmp: empleado;
        encontre: boolean;
    begin
        reset(arch);
        ...
        while (not EOF(arch) and (not encontre)) do begin
            read(arch, regEmp);
            if (regEmp.numEmpleado = numEmpleado) then  
                encontre:= true;
        end;

        if (encontre) then begin
            // me guardo la posicion del registro a eliminar y del ultimo registro del archivo
            posEncontrado := FilePos(arch) - 1;
            posUltimo := FileSize(arch) - 1;
            
            //me posiciono en el ultimo registro y lo guardo
            seek(arch, posUltimo);
            read(arch, regEmp);

            // me posiciono en el registro a eliminar y lo sobrescribo con el ultimo registro
            seek(arch, posEncontrado);
            write(arch, regEmp);
            ...
            writeln('Se ha eliminado el empleado: ', numEmpleado, ' con exito!');
        end
        else 
